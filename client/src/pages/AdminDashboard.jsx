...
export default function AdminDashboard() {
  ...
  // ✅ โลโก้ Age Group (upload)
  const [ageLogoFiles, setAgeLogoFiles] = useState({}); // { [ageGroupId]: File }
  const [ageLogoPreview, setAgeLogoPreview] = useState({}); // { [ageGroupId]: previewUrl }
  const [ageLogoUploading, setAgeLogoUploading] = useState({}); // { [ageGroupId]: boolean }

  // ✅ ปก Cate (upload)
  const [ageCateCoverFiles, setAgeCateCoverFiles] = useState({}); // { [ageGroupId]: File }
  const [ageCateCoverPreview, setAgeCateCoverPreview] = useState({}); // { [ageGroupId]: previewUrl }
  const [ageCateCoverUploading, setAgeCateCoverUploading] = useState({}); // { [ageGroupId]: boolean }

  // ✅ Category Icon (upload)
  const [catIconFiles, setCatIconFiles] = useState({}); // { [catId]: File }
  const [catIconPreview, setCatIconPreview] = useState({}); // { [catId]: previewUrl }
  const [catIconUploading, setCatIconUploading] = useState({}); // { [catId]: boolean }

  // ✅ ฟอร์มเพิ่ม/แก้ไขช่วงอายุ
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // cleanup preview โลโก้
      Object.values(ageLogoPreview || {}).forEach((u) => {
        try {
          if (u) URL.revokeObjectURL(u);
        } catch {}
      });

      // cleanup preview ปก cate
      Object.values(ageCateCoverPreview || {}).forEach((u) => {
        try {
          if (u) URL.revokeObjectURL(u);
        } catch {}
      });

      // cleanup preview ไอคอนวิชา
      Object.values(catIconPreview || {}).forEach((u) => {
        try {
          if (u) URL.revokeObjectURL(u);
        } catch {}
      });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (
        normalizedAges.length > 0 &&
        !normalizedAges.some((ag) => ag.ageValue === targetGroup)
      ) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.ageRange.length === 0)
      return alert("กรุณาเลือกระดับชั้นอย่างน้อย 1 ระดับ");
    if (form.categories.length === 0)
      return alert("กรุณาเลือกวิชาอย่างน้อย 1 วิชา");
    if (!isEdit && (!fileImg || !filePdf))
      return alert("กรุณาอัปโหลดรูปและ PDF ให้ครบ");
  ...
  const handleDeleteCat = async (catId) => {
    ...
    // เคลียร์ icon preview/file ของวิชานี้
    setCatIconFiles((prev) => {
      const next = { ...prev };
      delete next[catId];
      return next;
    });
  ...
  const ageOptions = ageGroups.length ? (
    <>
      {ageGroups.map((ag) => (
        <option key={ag.id} value={ag.ageValue}>
          {ag.label}
        </option>
      ))}
    </>
  ...
  const handlePickAgeLogo = (agId, file) => {
    if (!file) return;

    setAgeLogoPreview((prev) => {
      const old = prev?.[agId];
      if (old) {
        try {
          URL.revokeObjectURL(old);
      ...
      return { ...prev, [agId]: URL.createObjectURL(file) };
    });

    setAgeLogoFiles((prev) => ({ ...prev, [agId]: file }));
  };

  const handleUploadAgeLogo = async (ag) => {
    const file = ageLogoFiles[ag.id];
    if (!file) return alert("กรุณาเลือกไฟล์โลโก้ก่อน");

    setAgeLogoUploading((prev) => ({ ...prev, [ag.id]: true }));
    try {
      await API.uploadAgeGroupLogo(ag.id, file);

      const ages = (await API.getAgeGroups()) || [];
      setAgeGroups(ages.map(normalizeAge));

      setAgeLogoFiles((prev) => {
        const next = { ...prev };
        delete next[ag.id];
        return next;
      });
    ...
    setAgeLogoUploading((prev) => ({ ...prev, [ag.id]: false }));
  };

  const handlePickAgeCateCover = (agId, file) => {
    if (!file) return;

    setAgeCateCoverPreview((prev) => {
      const old = prev?.[agId];
      if (old) {
        try {
          URL.revokeObjectURL(old);
    ...
    setAgeCateCoverFiles((prev) => ({ ...prev, [agId]: file }));
  };

  const handleUploadAgeCateCover = async (ag) => {
    const file = ageCateCoverFiles[ag.id];
    if (!file) return alert("กรุณาเลือกไฟล์ปก Cate ก่อน");

    setAgeCateCoverUploading((prev) => ({ ...prev, [ag.id]: true }));
    try {
      await API.uploadAgeGroupCateCover(ag.id, file);

      const ages = (await API.getAgeGroups()) || [];
      setAgeGroups(ages.map(normalizeAge));

      setAgeCateCoverFiles((prev) => {
        const next = { ...prev };
        delete next[ag.id];
        return next;
      });
      ...
      console.error(err);
      alert("อัปโหลดปก Cate ไม่สำเร็จ");
    }
    setAgeCateCoverUploading((prev) => ({ ...prev, [ag.id]: false }));
  };
  ...
  const handleDeleteAgeGroup = async (ageId) => {
    ...
    try {
      ...
      // เคลียร์ preview/file ปก cate แถวนั้นด้วย
      setAgeCateCoverFiles((prev) => {
        const next = { ...prev };
        delete next[ageId];
        return next;
      ...
      // เคลียร์ preview/file โลโก้แถวนั้นด้วย
      setAgeLogoFiles((prev) => {
        const next = { ...prev };
        delete next[ageId];
        return next;
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
          ...
          <div className="flex gap-2 sm:gap-3">
            ...
            >
              <UploadCloud size={18} />
              <span className="hidden sm:inline">เพิ่มทีละหลายใบ</span>
            </button>

            <button
              onClick={() => setShowCatModal(true)}
              className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-3 sm:px-4 py-2 rounded-xl font-bold hover:bg-slate-50 hover:border-slate-300 transition-all text-xs sm:text-sm shadow-sm"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 text-emerald-700 px-3 sm:px-4 py-2 rounded-xl font-bold hover:bg-emerald-100 hover:border-emerald-300 transition-all text-xs sm:text-sm shadow-sm"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-rose-50 text-rose-600 border border-rose-100 px-3 sm:px-4 py-2 rounded-xl font-bold hover:bg-rose-100 transition-all text-xs sm:text-sm shadow-sm"
            >
              <LogOut size={18} />
          ...
          <div className="bg-white rounded-[2rem] p-6 shadow-lg border border-slate-100 sticky top-24">
            <h2 className="text-lg font-bold mb-6 flex gap-3 items-center text-slate-700 pb-4 border-b border-slate-100">
              <div
                className={`p-2 rounded-full ${
                  isEdit ? "bg-orange-100 text-orange-500" : "bg-indigo-100 text-indigo-500"
                }`}
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div
                  className={`relative border-2 border-slate-200 rounded-xl p-3 flex items-center justify-center cursor-pointer transition-all hover:border-indigo-400 hover:shadow-sm ${
                    filePdf || (isEdit && form.pdfUrl)
                ...
                >
                  ...
                  <span className="text-sm font-bold truncate max-w-[200px]">
                    {filePdf
                      ? filePdf.name
                      : isEdit && form.pdfUrl
                      ? "มีไฟล์เดิมแล้ว (คลิกเพื่อเปลี่ยน)"
        ...
        <div className="lg:col-span-8">
          <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            <h2 className="text-base font-bold text-slate-700 flex items-center gap-2">
              ...
              รายการใบงานทั้งหมด{" "}
              <span className="bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-md text-sm">
                {data.length}
              </span>
            </h2>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
              {data.map((d) => {
                ...
                return (
                  <div
                    key={d.id}
                    className="bg-white p-3 rounded-[1.5rem] shadow-sm border border-slate-100 hover:shadow-md hover:border-indigo-200 transition-all group flex flex-col relative"
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0 flex flex-col">
                      <div className="mb-1.5 flex flex-wrap gap-1">
                        {cats.slice(0, 2).map((cat, idx) => (
                          <span
                            key={idx}
                            className="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-md font-bold truncate block max-w-full border border-indigo-100"
                          >
                            {cat}
                          </span>
                        ))}
                        {cats.length > 2 && (
                          <span className="text-[9px] text-slate-400">+{cats.length - 2}</span>
                        )}
                      </div>

                      <h3
                        className="font-bold text-slate-700 text-sm line-clamp-2 mb-3 leading-tight flex-1"
                        title={d.title}
                      >
                        {d.title}
                      </h3>
        ...
        {showCatModal && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white p-6 rounded-[2rem] w-full max-w-md shadow-2xl relative border border-white/50"
            >
              <button
                onClick={() => setShowCatModal(false)}
                className="absolute top-5 right-5 bg-slate-100 p-2 rounded-full hover:bg-slate-200 transition-all text-slate-500"
              >
                <X size={20} />
              </button>
              ...
              <div className="mb-5 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <label className="text-xs font-bold text-slate-500 ml-1 mb-2 block uppercase tracking-wider">
                  เลือกกลุ่มที่ต้องการจัดการ
                </label>
                <div className="relative">
                  <select
                    className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 outline-none text-sm font-bold text-slate-700 appearance-none cursor-pointer transition-all"
                    value={targetGroup}
              ...
              <form onSubmit={handleAddCat} className="flex gap-3 mb-6">
                ...
                <button
                  type="submit"
                  className="bg-indigo-600 text-white px-4 rounded-xl font-bold hover:bg-indigo-700 shadow-md flex items-center justify-center transition-all transform hover:scale-105"
                >
                  <Plus size={20} />
                </button>
              </form>

              {/* ✅ รายการวิชา + อัปโหลดไอคอนรูป */}
              <div className="max-h-[300px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                {allCategories
                  ...
                  .map((cat) => {
                    ...
                    return (
                      <div
                        key={cat.id}
                        className="p-3 bg-white rounded-xl border border-slate-100 text-sm hover:shadow-md transition-all group"
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0 flex items-center gap-3">
                            ...
                            <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center shrink-0">
                              {catIconPreview[cat.id] || iconUrl ? (
                                <img
                                  src={catIconPreview[cat.id] || iconUrl}
                                  alt={cat.name}
                          ...
                          <button
                            ...
                            className="text-slate-400 hover:text-red-500 bg-slate-50 p-2 rounded-lg transition-all hover:bg-red-50 shrink-0"
                          >
                            <Trash2 size={18} />
                          </button>
                        </div>
                        ...
                        <div className="mt-3 flex items-center gap-2 flex-wrap">
                          <label className="text-[11px] font-bold px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 cursor-pointer hover:bg-slate-100">
                            เลือกไอคอน
                            <input
                              type="file"
                              accept="image/*"
                              className="hidden"
                              onChange={(e) => handlePickCatIcon(cat.id, e.target.files?.[0])}
                            />
                          </label>
                ...
                {allCategories.filter((c) => (c.ageGroup ?? c.age_group) === targetGroup).length === 0 && (
        ...
        {showAgeModal && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white p-6 rounded-[2rem] w-full max-w-lg shadow-2xl relative border border-white/50"
            >
              <button
              ...
              <div className="mb-5 max-h-48 overflow-y-auto space-y-2 pr-1">
                {ageGroups.map((ag) => (
                  <div
                    key={ag.id}
                    className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 gap-3"
                  >
                    {/* ซ้าย: รูป + ข้อมูล */}
                    <div className="min-w-0 flex items-start gap-3">
                      {/* รูป 2 อัน: โลโก้ + ปก Cate */}
                      <div className="flex gap-2 shrink-0">
                        <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center overflow-hidden">
                          {ageLogoPreview[ag.id] || ag.logoUrl ? (
                            <img
                              src={ageLogoPreview[ag.id] || ag.logoUrl}
                              alt={ag.label}
                              className="w-full h-full object-contain"
                            />
                          ) : (
                        ...
                        <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center overflow-hidden">
                          {ageCateCoverPreview[ag.id] || ag.cateCoverUrl ? (
                            <img
                              src={ageCateCoverPreview[ag.id] || ag.cateCoverUrl}
                              alt={`${ag.label} cover`}
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <label className="text-[11px] font-bold px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 cursor-pointer hover:bg-slate-100">
                            ...
                            <input
                              ...
                              accept="image/*"
                              className="hidden"
                              onChange={(e) => handlePickAgeLogo(ag.id, e.target.files?.[0])}
                            />
                          </label>

                          <button
                            type="button"
                            onClick={() => handleUploadAgeLogo(ag)}
                          ...
                          >
                            {ageLogoUploading[ag.id] ? "กำลังอัปโหลด..." : "อัปโหลดโลโก้"}
                          </button>

                          {ageLogoFiles[ag.id] && (
                            <span className="text-[10px] text-slate-400 truncate max-w-[160px]">
                              {ageLogoFiles[ag.id].name}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <label className="text-[11px] font-bold px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 cursor-pointer hover:bg-slate-100">
                            ...
                            <input
                              ...
                              accept="image/*"
                              className="hidden"
                              onChange={(e) => handlePickAgeCateCover(ag.id, e.target.files?.[0])}
                            />
                          </label>

                          <button
                            type="button"
                            onClick={() => handleUploadAgeCateCover(ag)}
                            disabled={!ageCateCoverFiles[ag.id] || ageCateCoverUploading[ag.id]}
                            className="text-[11px] font-bold px-2 py-1 rounded-lg bg-indigo-500 text-white hover:bg-indigo-600 disabled:opacity-60 disabled:cursor-not-allowed"
                          >
                            {ageCateCoverUploading[ag.id] ? "กำลังอัปโหลด..." : "อัปโหลดปก"}
                          </button>

                          {ageCateCoverFiles[ag.id] && (
                            <span className="text-[10px] text-slate-400 truncate max-w-[160px]">
                              {ageCateCoverFiles[ag.id].name}
                            </span>
                          )}
                        </div>
                      </div>
                ...
                {ageGroups.length === 0 && (
                  <div className="text-center py-8 text-slate-400 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 text-sm">
                    ยังไม่มีหมวดหมู่อายุในระบบ
                  </div>
                )}
              </div>
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={ageSaving}
                  className="w-full mt-1 bg-emerald-500 text-white py-2.5 rounded-xl font-bold text-sm hover:bg-emerald-600 transition-all disabled:opacity-60"
                >
        ...
        {preview && (
          ...
          >
            ...
            >
              <button
                onClick={() => setPreview(null)}
                className="absolute -top-4 -right-4 bg-white text-slate-400 hover:text-red-500 w-11 h-11 flex items-center justify-center rounded-full shadow-lg transition-all cursor-pointer hover:rotate-90 z-20 border-4 border-slate-50"
              >
              ...
              <div className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-inner relative overflow-hidden">
                <div className="text-center mb-6 mt-2">
                  ...
                  <h3 className="font-bold text-xl text-slate-800 leading-tight px-2 font-sans">
                    {preview.title}
                  </h3>
                </div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2 w-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all text-sm group"
                >
